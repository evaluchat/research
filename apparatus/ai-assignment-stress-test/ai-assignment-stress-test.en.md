...
description: "Apparatus #2: a teacher-facing, assignment-level stress test that probes what current AI can do, maps what the task measures, identifies delegation loss, and offers alternative designs without requiring AI prohibition."
tags: [apparatus, assessment, measurement-validity, ai, stress-test, teacher-tool]
...
provenance:
  sources:
    - id: ai-assignment-stress-test-design
      ...
      title: AI Assignment Stress Test — Apparatus #2 Design
profiles:
...
catalog_urls:
  spec: https://github.com/evaluchat/research/blob/main/apparatus/ai-assignment-stress-test/ai-assignment-stress-test.en.md
  evidence: https://github.com/evaluchat/research/tree/main/apparatus/ai-assignment-stress-test/evidence
  questions:
...
sources:
  ...
  - id: research-apparatus-concept
    resource: https://github.com/evaluchat/knowledge/blob/main/okf/bundles/evaluchat/concepts/research-apparatus.md
    title: Research Apparatus Platform (knowledge catalog)
  - id: apparatus-method
...
> `status: draft`. Apparatus #2 is a design specification, not an implementation. It is a
> teacher-side instrument for analysing an assignment as written; it is not a learner workflow,
> a submission surface, or an AI-use detector. It applies the [instrument view](../../methods/apparatus.en.md)
> to the assignment side of the measurement-validity question.

## Research question

This apparatus serves [AI assignment stress-test validity — when can task probes identify what an
...
loss across Modes A–F, and the costs and consequences of redesign alternatives. Apparatus #1's
...
artefact. The apparatus returns a structured assignment-level report. Its workflow is:
...
`min_canvas_version: 0.5.9` adopts the shared knob vocabulary. The five declared required
...
platform implements or executes this apparatus.

The apparatus uses the recipe's deliberate divergence: it needs neither class management nor a
...
`model_set` is pinned by apparatus version. Results are comparable only within the same pinned
model set, apparatus version, and resolved configuration. The `canonical-all-modes` profile is an
...
The report names the model set, apparatus version, resolved configuration, and Canvas version.
...
Evidence lives in the apparatus-owned [evidence collection](evidence/index.md). A report may be
...
apparatus:
  id: ai-assignment-stress-test
  version: 0.1.0
  configuration: # the resolved knob values, including the pinned model set
  canvas:
    version: <canvas-version>
```

The teacher's assignment is the sole input. Retention beyond the report requires the teacher's
opt-in publication decision; raw learner material does not belong in this apparatus or its evidence
collection.

## Status and versioning

- `status: draft`; no Canvas implementation, completed probe, or evidence contribution exists.
- Apparatus version `0.1.0` describes this design. A change to report semantics, the outcomes
  vocabulary, or a pinned model set is a comparability change and requires an apparatus-version
  decision.
- The apparatus version and Canvas version are independent and are both required in evidence
  provenance.
