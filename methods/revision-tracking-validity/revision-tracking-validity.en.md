...
description: "Method: a randomised, assessor-blind assignment study that evaluates the validity, fairness, limitations, and practical usefulness of condition-neutral revision metrics as evidence about recorded Canvas AI events."
tags: [method, revision-tracking, process-evidence, assessment-validity, fairness, privacy, ai]
...
> `status: draft`. This method is a research-method specification, not an implementation and not
> a detector. Its intended outcome is credible evidence about when revision tracking is useful,
> when it misleads, and whether it should influence an educational judgment at all. No method
...
> no server-side, per-assignment-group lever that randomly binds individual students to immutable
...
This method is the first public acceptance example for the linked feature request. The sequence
...
`blocked method` → `public feature request` → `versioned capability contract` →
...
The missing capability is sufficient to unblock the method only when this end-to-end flow can be
...
6. The reproducibility packet proves the method version, Canvas version, resolved profiles,
...
## 1. Method definition and rationale

This method runs an equivalent-task, randomised assignment study with an assessor-blind,
...
The method evaluates two related but non-interchangeable comparisons:
...
- Before recruitment, register the question, method version, profiles, task set, learning
...
feasibility phase reports process rates and uncertainty rather than claiming the method passes
...
|---|---|
| **Study definition** | Stores a study ID, method/version, protocol and ethics references, eligible population rule, task-set version, allocation protocol, profile identifiers, masking plan, metric schema, retention policy, and reproducibility-packet schema. It does not encode a cheating label. |
...
| **Reproducibility export** | Produces a versioned packet with method and Canvas versions, resolved profiles, lever hashes, allocation method, metric schema, display schema, analysis plan, data dictionary, retention/de-identification statement, and reveal-audit digest. Public export contains no student-identifiable text, transcripts, raw revision trace, or identity mapping. |
...
|---|---|---|
| `manifest` | Method ID/version, Canvas version/build, study ID, packet version, creation time, hashes, licences, and retention class. | Shareable after review. |
...
The packet records `method: {id, version, levers, canvas}` for every
...
### Non-claims enforced by the method
...
- The method does not create an automated integrity score, risk band, alert, disciplinary record,
  grade modifier, or admissions/advancement decision.
- Results apply only to the declared method version, Canvas version, profiles, tasks, participants,
  event definitions, and local prevalence. They do not validate a general detector.

### Failure conditions with useful results

The method must report non-endorsement for the proposed research use, or stop a phase pending
...
- Method version `0.1.0` fixes the proposed profile pair, two-pass method, metric schema label,
