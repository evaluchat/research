...
Evaluchat **runs** a method profile. It does not own research truth. Knowledge documents what each lever means on the platform; this catalog documents which levers an investigation uses and what evidence a concluded run must file.
...
The Revision-tracking method specifies immutable `canvas-ai-enabled` and `canvas-ai-disabled` profiles for an assessor-blind validity study. It is currently blocked by the missing, reusable [`experimental-assignment-condition` capability request](https://github.com/evaluchat/research/issues/1), which must provide server-side random allocation within a defined assignment group. It is a research-method proposal, not a detector or an integrity feature.
