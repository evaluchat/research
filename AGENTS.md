...
This catalog is a research layer, not a publication venue: open, versioned, auditable artifacts that academics can inspect, challenge, reuse, and extend. A public finding is downstream of a question, protocol, ethics decision, method, data, analysis, and independent human review. Never imply that an internal observation, product telemetry, or AI-generated report is peer-reviewed research. No claim may be stronger than the design, evidence, provenance, and review supporting it.

Governing documents: [contribution ladder](governance/contribution-ladder.en.md), [review protocol](governance/review-protocol.en.md), [shared evidence roles](governance/evidence-roles.en.md).

## Consuming

1. **Discover** — read `README.md` (human intro) then `index.md` (OKF section listing), then `catalog.json` (once built); filter by `type` / `tags` / `lang` / `status` / `stage`.
...
6. **Relationships** — traverse typed edges along the research chain: question → protocol/preregistration → ethics/consent → method → intervention → data/analysis → evidence → independent human review → finding; then challenge, replication, or knowledge update. Do not skip ethics, method identity, or review.
7. **Provenance** — cite `sources` plus `generated` / `verified`; attribute per claim. Record `method: {id, version, levers, canvas}` and data-sharing limits — including what cannot be independently reproduced.
...
7. **Do not hide uncertainty** — record null results, missing data, failed interventions, deviations, and competing explanations. Evidence must link a research question that predates it. Method and protocol versions are immutable after data collection; amend via a new version and an explicit change record.
8. **Canvas vs research** — method files specify the investigation (question, version, capabilities, levers, immutable profiles, telemetry, provenance). They never contain executable Canvas deployment code. Keep Canvas capability ownership separate from research method ownership.
