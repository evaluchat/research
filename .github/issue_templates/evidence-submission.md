---
name: Evidence submission
about: Submit an evidence contribution or discuss one — consent and anonymisation required
title: "[Evidence] <summary>"
labels: [evidence]
---

## Summary
<!-- One paragraph: what was done, where, with whom. -->

## Research question addressed
<!-- Link the question `id` this evidence bears on. It must predate the evidence. -->

## Ladder stage
<!-- observation | documented experience | structured experiment | replication/challenge | synthesis -->

## Files
<!-- List the evidence bundle files you intend to add under evidence/<slug>/. -->

## Consent & privacy
<!-- REQUIRED: describe the consent record and anonymisation. Raw student material is never committed. -->
